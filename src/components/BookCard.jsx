...
export default function BookCard({ image, title, rating, progress }) {
  return (
    <Card sx={{ display: "flex", alignItems: "center", mb: 2, p: 1 ,  maxWidth: 400}}>
      <CardMedia
        component="img"
      ...
      <CardContent sx={{ flex: 1 }}>
        <Typography variant="h5" color="body" fontWeight="bold">{title}</Typography>
        <Rating value={rating} precision={0.5} readOnly size="small" />
        <Box mt={1}>
          <Chip label={`${progress}%`} color="button" size="medium" />
        </Box>
